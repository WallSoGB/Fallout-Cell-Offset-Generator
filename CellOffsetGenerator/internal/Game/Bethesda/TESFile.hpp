...
public:
	...
	void Destroy(bool abDeallocate);
	...
	TESFile* GetThreadSafeFileForThread(uint32_t auiThreadID);

	TESFile* GetIndexFile(uint32_t auiIndex) const;

	bool MastersChanged() const;

	bool OpenTES(NiFile::OpenMode aeAccessMode, bool abLock);
	bool OpenTES(const char* apPath, const char* apFileName, NiFile::OpenMode aeAccessMode, bool abLock);
	bool CloseTES();
	uint32_t LoadTESInfo();
	void TESRewind(bool abGetForm);
	uint32_t GetTESForm();

	void CloseAllOpenGroups();

	void FreeDecompressedFormBuffer();

	bool ReadFormHeader();

	void LoadForm(TESForm* apForm);
	bool FindForm(const TESForm* apForm);
	...
	TESFile::FormInfo* GetCurrentForm();

	bool GetChunkData(void* apBuffer, uint32_t auiSize);

	bool SetOffset(uint32_t auiOffset);

	// Skips endian check
	template <typename T>
	bool GetChunkData(T& apData) { return GetChunkData(&apData, sizeof(T));}

	uint32_t GetChunkSize() const { return uiActualChunkSize; }
	uint32_t GetFormOffset() const { return uiFormOffset; }
	uint16_t GetFormVersion() const { return kCurrentForm.usFormVersion; }

	void ClearMasterLists();

	void RemoveFromOwnedIDMap();

	void FreeThreadSafeFileMap();

	bool GenIndexTable(BSSimpleList<TESFile*>* apFiles, bool abShowError);
	...
	void SomethingFlags(uint32_t auiFlags);
};
